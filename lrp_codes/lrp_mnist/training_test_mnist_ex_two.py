...
def train(nn, Xtrain, Ytrain, Xtest, Ytest, iters=10, save_file_name='../mnist_mlp-1296-1296-1296.txt'):
    
    nn.train(Xtrain, Ytrain, Xtest, Ytest, batchsize=25, iters=iters, status=10000)
    acc = np.mean(np.argmax(nn.forward(Xtest), axis=1) == np.argmax(Ytest, axis=1))
    if not np == numpy: # np=cupy
        acc = np.asnumpy(acc)
    print('model test accuracy is: {:0.4f}'.format(acc))
    model_io.write(nn, save_file_name)
    #'''
    #try loading the model again and compute score, see if this checks out. this time in numpy
    nn = model_io.read(save_file_name)
    acc = np.mean(np.argmax(nn.forward(Xtest), axis=1) == np.argmax(Ytest, axis=1))
    if not np == numpy: acc = np.asnumpy(acc)
    print('model test accuracy (numpy) is: {:0.4f}'.format(acc))
    

def test(nn, Xtest, Ytest, iters=10, save_file_name='../mnist_mlp-1296-1296-1296.txt'):
...
if train_mnist:
    ...
    prob =0.10
    Xtrain = add_salt_and_pepper_noise(prob, Xtrain)
    Xtest = add_salt_and_pepper_noise(prob, Xtest)
    
    
    Xtrain = create_preprocessing(Xtrain, input_range=[-1, 1])
    Xtest = create_preprocessing(Xtest, input_range=[-1, 1])
    ...
    #'''
    nn_tanh = modules.Sequential(
        [
            modules.Flatten(),
            modules.Linear(1024, 1296),
            modules.Tanh(),
    ...
    #train(nn_tanh, Xtrain, Ytrain, Xtest, Ytest, iters=1000000, save_file_name='../mnist_mlp_big_tanh-1296-1296-1296.txt')
    #test(nn_tanh, Xtest, Ytest, iters=1000000, save_file_name='../mnist_mlp_big_tanh-1296-1296-1296.txt')
    
    #train(nn_relu, Xtrain, Ytrain, Xtest, Ytest, iters=1000000, save_file_name='../mnist_mlp_big_relu-1296-1296-1296.txt')
    #test(nn_relu, Xtest, Ytest, iters=1000000, save_file_name='../mnist_mlp_big_relu-1296-1296-1296.txt')
    
    train(nn_relu, Xtrain, Ytrain, Xtest, Ytest, iters=10000, save_file_name='../mnist_mlp_small_2_relu-1296-1296-1296.txt')
    test(nn_relu, Xtest, Ytest, iters=10000, save_file_name='../mnist_mlp_small_2_relu-1296-1296-1296.txt')
