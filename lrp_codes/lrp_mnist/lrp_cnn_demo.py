...
colab_path = '/content/drive/My Drive/0_official-laptop-data/course_materials/Deep Learning,_Advanced_Course/deep_learning_project/reference_codes/lrp_toolbox'


#load a neural network, as well as the MNIST test data and some labels
nn = model_io.read(colab_path+'/models/MNIST/LeNet-5.nn') # 99.23% prediction accuracy
nn.drop_softmax_output_layer() #drop softnax output layer for analyses

X = data_io.read(colab_path+'/data/MNIST/test_images.npy')
Y = data_io.read(colab_path+'/data/MNIST/test_labels.npy')
...
for i in I[:10]:
    ...
    digit_hm = render.save_image([digit,hm],colab_path+'/heatmap.png')
    data_io.write(R,colab_path+'/heatmap.npy')
...
if True:
    ...
    y = nn.forward(x)
    R = nn.lrp(y)
    data_io.write(R, colab_path+'/Rbatch.npy')
